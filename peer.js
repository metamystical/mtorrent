...
Peer.prototype.socket = function (c) {
  if (!c) {
    c = new tcp.Socket()
    const opts = { port: this.port, host: this.address }
    opts.localPort = this.localPort
    c.connect(opts)
  }
  c.once('error', this.onError.bind(this)) // triggers automatic close
  c.once('close', this.onFinish.bind(this))
  c.on('data', (chunk) => {
...
Peer.prototype.onMessage = function (cmd, args) {
  switch(cmd) {
    ...
    case 'onRequest':
      ...
      this.write(sendBlock(args.inx, args.begin, pieceData.slice(args.begin, args.begin + args.length)))
      break
    case 'onBlock':
    ...
    case 'onGetMetaMessage':
      if (args.metaMsgType === 0) { // request
        if (this.metaPiece > Math.ceil(this.metaInfoSize / META_PIECE_SIZE)) this.abort('metadata request')
...
Peer.prototype.onError = function (err) {
  // console.log('socket error: ' + err) // debug
}

Peer.prototype.onFinish = function (hadErr) {
  // console.log('connection closed' + (hadErr ? ' with error' : '')) // debug
